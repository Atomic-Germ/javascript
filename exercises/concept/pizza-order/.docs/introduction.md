...
If a base case is missing, it will call itself indefinitely.
Realistically, you'll end up with a "StackSize error": an error raised by the runtime because the _stack_ of function calls has grown beyond a predefined limit. This happens because each recursive call adds to this _stack_ until it returns (and it doesn't).
